...
\begin{itemize}
    ...
    \item $\bth_k,\tau_k$ selected randomly (hence random tree) % more precisely, a set of features {theta1,...thetax} is generated randomly, and the best thetak are selected from this set (best feature on top, and so on)
\end{itemize}
...
Can be accomplished using a constellation model, for example % not sure how the Kinect does it, did not find accurate information
