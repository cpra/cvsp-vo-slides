...
\begin{itemize}
	\item Requires good initial estimate of $\bth$ (local optimization) % ICP is a typical example of an iterative optimization problem. as such, it is crucial that the initial parameter estimates are close to the global minimum, otherwise the optimization gets stuck in a local minimum. we have discussed this before, and it is nicely visible in the below video.
\end{itemize}
...
\begin{center}
    ...
    {\centering Image adapted from \cite{berger2014}} % nice paper on current surface reconstruction method
\end{center}
