...
\begin{itemize}
	\item Usually involves combing multiple point clouds % because there are holes due to occlusions / data from point clouds / depth maps / disparity maps ... these are treated as equal because we can map between them ... see previous slide set
\end{itemize}
...
\begin{itemize}
    \item Using marching cubes, for example % we define a voxel grid with some resolution and then loop through it voxel by voxel. at each voxel we check the neighborhood for what of the below conditions apply using simple in/out tests, and create triangles accordingly
\end{itemize}
...
\begin{center}
    \copyrightbox[b]
    {\includegraphics[width=7cm]{figures/david-model.jpg}} % rendering of a 3d model of Michelangelo's David, obtained using a laser scanner. there are now reconstruction methods that lead to a similar accuracy while using only high resolution images
...
\begin{itemize}
    \item Measurements represent object distances % so what we measure is intuitive. for example, foreground must be closer than background
...
\begin{itemize}
    \item Object size, extent, volume, shape, ... % some invariant only if we know the extrinsics (like height)
\end{itemize}
...
\begin{center}
	\url{https://www.youtube.com/watch?v=oU4XQvxO10k} % the Mercedes also uses radar for this, for longer range and probably during nighttime. it's also just one example, most manufacturers now have some form of this in their premium models
\end{center}
...
For each pixel $\vx$ with depth $d(\vx)$ compute $\Pr(w|\vx)$ % d(x) = depth as stored in depth map at pixel x, which corresponds to w in the previous slide set (i.e. not the Euclidean distance)
\begin{itemize}
...
Implemented using a \emph{random forest} % this is a powerful example of an ensemble method, see Bishop's book for more information
...
\begin{itemize}
    ...
    \item Until a leaf node is reached, which stores $\Pr_t(w|\vx)$ % we get Pr by simply passing each training sample through the final tree and collecting counts for each leaf node and w
\end{itemize}
...
\begin{itemize}
    \item This training \& inference strategy is called \emph{bagging} % we train multiple classifiers from subsets of the training data and then combine their predictions
\end{itemize}
