...
\begin{itemize}
    \item VGA resolution, $M=1000$: $\sim300$ million params % we don't have enough images to train such nets without running into overfitting problems
\end{itemize}
...
Each map consists of $M$ hidden units with \emph{shared} weights\\\medskip
Hidden units have a local receptive field % i.e. connected to only a local 
...
% H2: 5x5x8 receptive field (3D convolution), 12 feature maps, 2x2 pooling as above
...
Results of 2014 object detection challenge (excerpt) % see above for object detection definition
